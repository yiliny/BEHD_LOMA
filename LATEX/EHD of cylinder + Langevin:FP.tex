...
\beq \ddot\Delta = (a_1 \frac{\dot{\Delta}}{\Delta^{3/2}} + a_2 \frac{\dot{\Delta}^2}{\Delta^{9/2}} + a_3 \frac{\dot\Theta^2}{\Delta^{7/2}} + a_3 \frac{\dot{X}^2}{\Delta^{7/2}} + a_4 \frac{\dot\Theta \dot{X}}{\Delta^{7/2}} + a_6) \times \frac{-1}{1 + a_5 / \Delta^{7/2}} \tlag{Yilin.3} \eeq
...
\beq \dot{v}_i = F_i + TF^{drift}_i + \eta_i \tlag{Yilin.28} \eeq
...
\ssc{Mass vector}
In the previous parts, we always regarded the mass as 1. To be exact, the masses we would use in the Newtonian equation should be carefully distinguished. For $z,x$ components, $m_\alpha = m$ namely the mass of the column. However, for $\Theta$ one, $m_\alpha = mr^2 /2$ refers to the moment of inertia. Taking that into account, we compare these two versions (Note we re-write $U_\alpha$ as $F_\alpha$ on the left side)
...
\beq - \frac{F_{hZ}}{m_Z} = - v_z = - \ddot{\Delta} = a_1 \frac{\dot{\Delta}}{\Delta^{3/2}} + a_2 \frac{\dot{\Delta}^2}{\Delta^{9/2}} + a_3 \frac{\dot\Theta^2}{\Delta^{7/2}} + a_3 \frac{\dot{X}^2}{\Delta^{7/2}} + a_4 \frac{\dot\Theta \dot{X}}{\Delta^{7/2}} + a_5 \frac{\ddot\Delta}{\Delta^{7/2}} + a_6 = 0 \tlag{Yilin.34} \eeq
thus
\beq \bgn
...
\beq - \frac{F_{hX}}{m_X} = - v_x = - \textcolor{red}{\ddot{X}} = b_1 \frac{\dot{X}}{\sqrt\Delta} + b_2  \frac{\dot\Delta \dot{X}}{\Delta^{7/2}} + b_3 \frac{\dot\Delta \dot\Theta}{\Delta^{7/2}} + b_4 \frac{\textcolor{blue}{\ddot{\Theta}}}{\Delta^{5/2}} + b_5 \frac{\textcolor{red}{\ddot{X}}}{\Delta^{5/2}} + b_6 \tlag{Yilin.36} \eeq
\beq \bgn
...
and
\beq - \frac{F_{h\Theta}}{m_\Theta} = - v_\theta = - \textcolor{blue}{\ddot{\Theta}} = c_1 \frac{\dot\Theta}{\sqrt\Delta} + c_2 \frac{\dot\Delta \dot\Theta}{\Delta^{7/2}} + c_3 \frac{\dot\Delta \dot{X}}{\Delta^{7/2}} + c_4 \frac{\textcolor{red}{\ddot{X}}}{\Delta^{5/2}} + c_5 \frac{\textcolor{blue}{\ddot{\Theta}}}{\Delta^{5/2}} + c_6 \tlag{Yilin.38} \eeq
\beq \bgn
...
\ssc{Spurious drift} \label{sdrift}
