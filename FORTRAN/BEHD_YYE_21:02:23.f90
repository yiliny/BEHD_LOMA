...
Program main
    ...
    integer :: ratio_sim,ratio_msd,ratio_pdf,pdft_max,ratio_x!,num_particle,zfix !! Constant
    real*8 :: time_begin,time_end,temps_debut,temps_fin,time_sim,zmax,xmax,pdf_trapz !! define variables to record time consumed
    real*8,external :: rectgauss,Minverse,gammavalue,spurious,repulsion
    ...
    real*8 :: itm_p(3,2),itm_v(3,2),itm_f(2),itm_g(6) !! Define intermediates to facilitate the codes
    ...
    integer,allocatable :: pdfz(:),pdft(:,:),pdfx(:,:) !! pdfz for the single particle's time average. pdft for particle ensemble average at different time. 20228022 add pdfx
    ...
    continue !! Here we list all format declarations.
        ...
        64 format(2x,'Time',12x,'mass',12x,'Force z',12x,'Noise z',12x,'Noise x',12x,'Noise Θ')
        ...
        74 format(f16.9,5(5X,ES15.6)) !! format for file(34)=force_z.txt
        ...
        94 format('BEHD @ YYE. Simulation done on ',I4,2('-',I2),'   ',I2,'h',I2)
    ...
    continue !! Constant Parameters
        ...
        clight = sqrt(2.d0*grav*rayon*(1.0d0-rhosol/rhosty)) !! unit m/s, the maximum speed, 1.3e-3
        csquare = clight**2
        if (i.eq.1) then
        ...
        dt = 0.20d-4 * clight / (rayon * sqrt(2.0d0 * eps)) !* tratio

        !! Define loop parameters
        ratio_sim = 1; ratio_msd = 400 !! Here are two variables to accelerate calculations for Euler method & MSD.
        zmax = 1.8d0 !! Define the maximum height considered for PDF
        xmax = 1.0d0 !! 20220822 added. Define the maximum absolute displacement considered for PDF along x.
        ratio_pdf = ceiling(zmax*400) !! Here is the ratio to determine how many data would be taken part for PDF calculation.
        ratio_x = ceiling(2*xmax*50)
        pdft_max = 200 !! Define the maximum time section number. We ignore the further development after this value.
    ...
    kxi = kappa*xi; kxe = kappa*xi*eps !! non-dimensional parameters w/o units.
        ...
        case(1) !! Loop
            ...
            write(31,96) kappa, ini_height
            open(unit=32,file=TRIM(rue_total)//"positions.txt"); write(32,94) date(1), date(2), date(3), date(5), date(6)
            write(32,96) kappa, ini_height
            open(unit=33,file=TRIM(rue_total)//"velocitys.txt"); write(33,94) date(1), date(2), date(3), date(5), date(6)
            write(33,96) kappa, ini_height
            open(unit=34,file=TRIM(rue_total)//"forces.txt"); write(35,94) date(1), date(2), date(3), date(5), date(6)
            write(34,96) kappa, ini_height
            ...
            write(381,96) kappa, ini_height
            open(unit=382,file=TRIM(rue_total)//"PDFz.txt"); write(382,94) date(1), date(2), date(3), date(5), date(6)
            write(382,96) kappa, ini_height
            open(unit=383,file=TRIM(rue_total)//"PDF_time.txt"); write(383,94) date(1), date(2), date(3), date(5), date(6)
            write(383,96) kappa, ini_height
            open(unit=384,file=TRIM(rue_total)//"PDF_x.txt"); write(384,94) date(1), date(2), date(3), date(5), date(6)
            write(384,96) kappa, ini_height
            !! Furnish Variable Description.
            write(31,61)
            write(32,62); write(33,63); write(34,64); write(381,681); write(382,682); write(383,683); write(384,684)
            ...
            allocate(pdfx(ratio_x,pdft_max)) !! 20220822 added. For ratio_x = ceiling(2*xmax*50), 2 refers to ± two directions; xmax, the max absolute displacement; 50, interval number for unit length.
            ...
            itm_p = 0.0d0; itm_v = 0.0d0; itm_f = 0.0d0; itm_v(1,2) = -0.0d0
            ...
            do i = 1 , ratio_sim * (Nmax - 1)
                ...
                !! 03/02/23 added
                if (fnoise.eq.0) then ! w/o soft effect
                    amplitude(1,1)=Minv(1,1)*sqrt(4.0d0/beta*mz*(gmaeff(1,1,1)) / (Lchrt * csquare * eps))
                    !amplitude(2,2)=Minv(2,2)*sqrt(2.0d0/beta*mx*(gmaeff(1,2,2)) / (rayon * csquare))
                    amplitude(2,2)=Minv(2,2)*sqrt(4.0d0/beta*mx*(gmaeff(1,2,2)) / (Lchrt * csquare * eps))
                    !amplitude(3,3)=Minv(3,3)*sqrt(2.0d0/beta*mt*(gmaeff(1,3,3)) * rayon / csquare)
                    amplitude(3,3)=Minv(3,3)*sqrt(4.0d0/beta*mt*(gmaeff(1,3,3)) / (Lchrt * csquare * eps))
                end if
                if (fnoise.eq.1) then ! order-1 soft effect
                    amplitude(1,1)=Minv(1,1)*sqrt(4.0d0/beta*mz*(gmaeff(1,1,1)-gmaeff(2,1,1)) / (Lchrt * csquare * eps))!*0.d-6
                    !amplitude(2,2)=Minv(2,2)*sqrt(2.0d0/beta*mx*(gmaeff(1,2,2)-gmaeff(2,2,2)) / (rayon * csquare))!*1.d-6
                    amplitude(2,2)=Minv(2,2)*sqrt(2.0d0/beta*mx*(gmaeff(1,2,2)-gmaeff(2,2,2)) / (Lchrt * csquare * eps))
                    !amplitude(3,3)=Minv(3,3)*sqrt(2.0d0/beta*mt*(gmaeff(1,3,3)-gmaeff(2,3,3)) * rayon / csquare)!*1.d-6
                    amplitude(3,3)=Minv(3,3)*sqrt(2.0d0/beta*mt*(gmaeff(1,3,3)-gmaeff(2,3,3)) / (Lchrt * csquare * eps))
                end if
                if (fnoise.eq.2) then ! order1v soft effect, not verified
                    amplitude(1,1)=Minv(1,1)*sqrt(4.0d0/beta*mz*(gmaeff(1,1,1)-gmaeff(2,1,1)+gmaeff(3,1,1))/(Lchrt*csquare*eps))
                    !amplitude(2,2)=Minv(2,2)*sqrt(2.0d0/beta*mx*(gmaeff(1,2,2)-gmaeff(2,2,2)+gmaeff(3,2,2)) / (rayon * csquare))
                    amplitude(2,2)=Minv(2,2)*sqrt(2.0d0/beta*mx*(gmaeff(1,2,2)-gmaeff(2,2,2)+gmaeff(3,2,2))/(Lchrt*csquare*eps))
                    !amplitude(3,3)=Minv(3,3)*sqrt(2.0d0/beta*mt*(gmaeff(1,3,3)-gmaeff(2,3,3)+gmaeff(3,3,3)) * rayon / csquare)
                    amplitude(3,3)=Minv(3,3)*sqrt(2.0d0/beta*mt*(gmaeff(1,3,3)-gmaeff(2,3,3)+gmaeff(3,3,3))/(Lchrt*csquare*eps))
                end if


                !force(i) = forcevalue(position(1,i))
                itm_f(1) = repulsion(itm_p(1,1))
                itm_f(2) = spurious(itm_p(1,1))
                !fext(1) = forcevalue(position(1,i))
                !fext(1) = force(i)!; 
                fext(1) = itm_f(1) + itm_f(2) - mass; fext(2)=0.0d0; fext(3)=0.0d0
                ...
                if (tfix.eq.0) itm_p(3,2) = itm_p(3,1) !! Fix Θ all the time if necessary. Added on 20220823 same purpose above.
                ...
                if (MOD(i,ratio_sim).eq.0) then
                    ...
                    pdfz(j) = pdfz(j) + 1
                    !! Calcul PDF_t
                    if (count_sim.le.pdft_max) pdft(j,count_sim) = pdft(j,count_sim) + 1

                    !! Calculate PDF_x time-depending distribution
                    j = ceiling(ratio_x * (itm_p(2,2) + xmax) * 0.5d0 / xmax) !! itm_p = - xmax, j = ratio_x
                    !if (j.le.1) j = 1
                    !if (j.ge.ratio_x) j = ratio_x
                    if ((j.ge.1).and.(j.le.ratio_x)) then
                        if (count_sim.le.pdft_max) pdfx(j,count_sim) = pdfx(j,count_sim) + 1
                    end if
            ...
            pdf_trapz = Nmax * zmax/ratio_pdf
            do l=1,pdft_max
                write(383,*) ""; write(383,*) ""
                write(383,3831) l !! Furnish INDEX
                do k=1,ratio_pdf
                    write(383,3832,advance='yes') zmax*(k - 0.5d0)/ratio_pdf, 1.0d0*pdft(k,l)/pdf_trapz !1.0d0*(pdft(k,l)-0.0d0)/num_particle 
                ...
                do k=1,ratio_x
                    write(384,3832,advance='yes') xmax*(k - 0.5d0)/ratio_x*2.0d0-xmax, 1.0d0*pdfx(k,l) !! X position/parallel displacement; particle number;
                end do
            end do
            call cpu_time(time_end)
            ...
            deallocate(sumx); deallocate(sumt); deallocate(sumz); deallocate(pdft); deallocate(pdfx) !deallocate(Dcoefx); deallocate(Dcoeft)
            !close(35); close(36); close(371); close(372); close(373); 
            close(31); close(32); close(33); close(34)
            close(381); close(382); close(383); close(384)
...
subroutine obtaindata(i)
    ...
    case(1)
        ...
        read(201,*) Lchrt !! Typical length \mathcal{L}
...
real*8 function gammavalue(no,i,j,z,vz,vx,vt)
    ...
    case(2) !! gamma_1
        ...
        !!!! 20220823 add for sake of precisement 
        if (i.eq.1) then
            if (j.eq.1) then
                gammavalue = (15.0d0*kxi*xi)/(8.0d0*z**5)
            else
                gammavalue = 0.0d0
            end if
        end if
        if (i.eq.2) then
            if (j.eq.1) gammavalue = 0.0d0
            if (j.eq.2) gammavalue = (kxi*xi*eps**2)/(18.0d0*z**3)
            if (j.eq.3) gammavalue = -(kxi*xi*eps**2)/(9.0d0*z**3)
        end if
        if (i.eq.3) then
            if (j.eq.1) gammavalue = 0.0d0
            if (j.eq.2) gammavalue = -(kxi*xi*eps**2)/(9.0d0*z**3)
            if (j.eq.3) gammavalue = (2.0d0*kxi*xi*eps**2)/(9.0d0*z**3)
        end if
    case(3) !! gamma_1v
...
!This function would furnish the force on each direction
    ...
    B = 3.0d4 * 1.0d0 !! namely the "B" shown by Maxime, PHYSICAL REVIEW RESEARCH 3, L032011 (2021)
    !! Attention to the unit! As for the force, it should be [kg·m·s^{-2}]
    !! However, since m = rho * π * r**2 = [kg/m], the force unit would be [kg/s^2]
    !! Finally, we take all parameters dimensionless.
    repulsion = ( 1.0d0/beta * B/l_D * exp(-Delta*rayon*eps/l_D) ) * (2.0d0*rayon)/(Lchrt*csquare)
    return
