...
Program main
    use MATHS
    implicit none    
    integer :: Nmax = 10000*10 !! Define the maximum number of steps. Note ∆t=0.1ms,Nmax=1e5,T->100
    integer :: dtmax = 8000*10 !! Define the maximum ∆t to calculate MSD
    integer i,j,k,l,count_kappa,count_delta,ratio_sim,ratio_msd !! loop variables
    ...
    real*8 :: itm_p(3,2),itm_v(3,2),itm_f
    ...
    78 format(f16.9,6(5X,ES10.3)) !! format for file(38-)
    ...
    count_kappa = 0; count_delta = 0 !! Here are two variables to count how many loops have passed.
    ratio_sim = 10; ratio_msd = 40 !! Here are two variables to accelerate calculations for Euler method & MSD.
    ...
    do i = 1 , Nmax - 1
        ...
        amplitude(1,1) = Minv(1,1) * sqrt(2.0d0/beta * mz * (gmaeff(1,1,1) - gmaeff(2,1,1)) * 2.0d0 / (rayon * clight**2 * eps))
        !amplitude(2,2) = amplitude(2,2) * clight
        !amplitude(2,2) = 1.0d0/clight * sqrt(2.0d0*(gmaeff(1,2,2) - gmaeff(2,2,2))/(beta*mx))
        amplitude(2,2) = Minv(2,2) * sqrt(2.0d0/beta * mx * (gmaeff(1,2,2) - gmaeff(2,2,2)) * 1.0d0 / (rayon * clight**2))
        !amplitude(3,3) = amplitude(3,3) * rayon / clight
        !amplitude(3,3) = rayon/clight * sqrt(2.0d0*(gmaeff(1,3,3) - gmaeff(2,3,3))/(beta*mt))
        amplitude(3,3) = Minv(3,3) * sqrt(2.0d0/beta * mt * (gmaeff(1,3,3) - gmaeff(2,3,3)) * rayon**2 / (clight**2 * rayon))
        ...
        Dcoefx(i) = (1.0d0 - gmaeff(2,2,2)/gmaeff(1,2,2))/(beta*mx*gmaeff(1,2,2)) * (1.0d0/clight)**2
        ...
        write(32,72) time_sim,position(1,i),position(2,i),position(3,i)
        write(33,73) time_sim,velocity(1,i),velocity(2,i),velocity(3,i)
        write(34,74) time_sim, Minv(1,1)*force(i), amplitude(1,1)*noise(1,1), amplitude(2,2)*noise(2,1), amplitude(3,3)*noise(3,1)
    ...
    do i=1,dtmax !! ∆t?
        ...
        write(381,78) time_sim, msdx_ana, sumx(i)/j, Dcoefx(i)*i*1.0d0, msdt_ana, sumt(i)/j, Dcoeft(i)*i*1.0d0
    end do
    1104 continue
    call cpu_time(time_end)
...
real*8 function forcevalue(z)
    ...
    & 1.0d0/beta * (gzzz+gzxx+gztt)/(rayon*eps) * (2.0d0 * rayon) / (rayon * clight**2) + &
    & coulombmax/(rayon * beta * l_D) * exp(-z*rayon*eps/l_D) * 2.0d0 / clight**2
