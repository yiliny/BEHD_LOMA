...
Program main
    ...
    real*8,external :: rectgauss,Minverse,gammavalue,spurious_force,repulsion,Laguerre,GBdist,GBvites,Mass_eff
    ...
    real*8 :: itm_q(3,2),itm_v(3,2),itm_f(2),itm_p(3,2),zsqrt !! Define intermediates to facilitate the codes. 20230911, Position->itm_q & Momenta->itm_p
    ...
    continue !! Here we list all format declarations.
        ...
        72 format(f11.5,3(",",4X,ES18.8)) !! format for file(32)=position_z.txt
    ...
    continue !! Constant Parameters
        ...
        mass = 4.0d0/3.0d0*pi*rayon**3*rhosty !! unit kg/m
        ...
        zmax = 4.0d0 !! Define the maximum height considered for PDF
        xmax = 1.0d0 !! 20220822 added. Define the maximum absolute displacement considered for PDF along x.
        vzmax = 1.5d0; vxmax = 0.9d0 !! 20230301 added for velocity distribution
        ratio_pdfz = ceiling(zmax*40) !! Here is the ratio to determine how many data would be taken part for PDF calculation.
        ratio_pdfx = ceiling(2*xmax*50)
        ratio_vz = ceiling(vzmax*100); ratio_vx = ceiling(vxmax*80) !! 20230301 added for velocity distribution
    ...
    aint = 0.0d0; node = 40 !! Add 20230228: Integration for normalized factor A of PDF
    do k=1,node
    ...
    do k=1,ratio_vz
        ...
        Pvana(k) = GBvites(vseq(k)*clight)
    end do
    aint = 0.0d0; node=60
    do k=1,node
        tvl=Laguerre(node,1,k); wi=Laguerre(node,2,k)
        aint = aint + wi * GBvites(tvl*clight) * exp(tvl) * clight !! \int P(v) dv = \int P(V*c) dV * c = c * \int P(V*reff)*exp(±V) dV 
    ...
    kxi = kappa*xi; kxe = kappa*xi*eps !! non-dimensional parameters w/o units.
        ...
        case(1) !! Loop
            ...
            write(30,96) kappa, ini_height; write(30,*) ""
            ...
            gmaeff=0.0d1; Minv=0.0d0; fext=0.0d0
            ...
            do i = 1 , ratio_sim * (Nmax - 1)
                ...
                amplitude(1,1) = sqrt(2.0d0/beta*gmaeff(1,1,1)/mass)
                fext(1) = repulsion(itm_q(1,1))
                !write(*,*) "mass =",mass,"force =",fext(1),"height",itm_q(1,1)
                ...
                itm_v(1,2) = itm_v(1,1) + amplitude(1,1) * noise(1,1) + dt * (fext(1) / mass - gmaeff(1,1,1) * itm_v(1,1)) 
                ...
                itm_q(1,2) = itm_q(1,1) + itm_v(1,1) * dt !! 20230913 We must use updated velocity to calculate the next position!
...
real*8 function repulsion(Delta)
    ...
    !! However, since m = rho * π * r**2 = [kg/m], the force unit would be [kg/s^2]
    !! Finally, we take all parameters dimensionless.
    repulsion = kk * (q0 - Delta) - mass * grav * (1.0d0 - rhosol / rhosty)
...
!***** Gauss-Laguerre Integration ***** 20230228 added from ntest.f90，用以单边无穷积分；
