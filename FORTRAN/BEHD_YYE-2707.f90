...
Program main
    use MATHS
    implicit none    
    integer :: Nmax = 6000*1 !! Define the maximum number of steps. Note ∆t=0.1ms,Nmax=1e5,T->100
    integer :: dtmax = 4000*1 !! Define the maximum ∆t to calculate MSD
    integer i,j,k,l,count_kappa,count_delta,count_sim,count_msd,crash,count_particle !! Loop variables
    integer :: ratio_sim,ratio_msd,ratio_pdf,num_particle,pdft_max,zfix !! Constant
    ...
    real*8 :: msdx_num,msdx_ana!,msdt_num,msdt_ana,msdc_ana !! Define three analytical MSD, <∆r_x **2> & <∆r_Θ **2> & <∆r_x * ∆r_Θ>
    ...
    continue !! Here we list all format declarations.
        ...
        681 format(2x,'Time',9x,'sumx',9x,'<∆x**2>',9x,'ana. MSD_x',9x,'sumt',9x,'<∆Θ**2>',9x,'ana. MSD_t')
        ...
        781 format(f13.3,6(4X,ES13.6)) !! format for file(381)=MSD.txt
        ...
        980 format(f8.2,' seconds spent for MSD calculations.')
        981 format(f8.2,' seconds spent for MSD output.')
    ...
    !1107 continue !! Constant Parameters
        ...
        ratio_sim = 300; ratio_msd = 8 !! Here are two variables to accelerate calculations for Euler method & MSD.
        ...
        num_particle = 80 !! Define the number of particles. We would repeat the same simulation to obtain the average property.
        pdft_max = 1000 !! Define the maximum time section number. We ignore the further development after this value.
        zfix = 0 !! Determine whether we fix the vertical coordinate. 0, fix at the initial height all the time; 1, not fixed with constant initial height; 2, not fixed with random height
    !1108 continue
    ...
    kxi = kappa*xi; kxe = kappa*xi*eps !! non-dimensional parameters w/o units.
        ...
        case(1) !! Loop
            ...
            if ((ini_height.eq.0.1d0).or.(ini_height.eq.0.3d0).or.(ini_height.eq.1.0d0)) then
                continue
            else
            ...
            do i=1,dtmax !! Loop for ∆t
                if ((MOD(i,ratio_msd).eq.0).or.(i.le.10)) then !! Accelerate the MSD calculs.
            ...
            !write(381,*) ""
            do l=1,dtmax
                if ((MOD(l,ratio_msd).eq.0).or.(l.le.10)) then !! Only output non-zero results.
                    ...
                    &sumt(l), 1.0d0*sumt(l)/(Nmax-l), l*1.0d0
                end if
            end do
            call cpu_time(time_end)
            ...
            deallocate(sumx); deallocate(sumt); deallocate(pdft) !deallocate(Dcoefx); deallocate(Dcoeft)
