...
Program main
    use MATHS
    implicit none    
    integer :: Nmax = 100*1 !! Define the maximum number of steps. Note ∆t=0.1ms,Nmax=1e5,T->100
    integer :: dtmax = 400*1 !! Define the maximum ∆t to calculate MSD
    integer i,j,k,l,count_kappa,count_delta,count_sim,count_msd,crash,count_particle !! Loop variables
    integer :: ratio_sim,ratio_msd,ratio_pdf,num_particle,pdft_max !! Constant
    ...
    real*8 :: msdx_num,msdt_num,msdx_ana,msdt_ana,msdc_ana !! Define three analytical MSD, <∆r_x **2> & <∆r_Θ **2> & <∆r_x * ∆r_Θ>
    ...
    real*8,allocatable :: sumx(:),sumt(:),Dcoefx(:),Dcoeft(:)!,msdx_num(:),msdt_num(:),
    ...
    1107 continue !! Constant Parameters
        ...
        ratio_sim = 80; ratio_msd = 2 !! Here are two variables to accelerate calculations for Euler method & MSD.
        ratio_pdf = 80 !! Here is the ratio to determine how many data would be taken part for PDF calculation.
        num_particle = 10000 !! Define the number of particles. We would repeat the same simulation to obtain the average property.
        pdft_max = 150 !! Define the maximum time section number. We ignore the further development after this value.
    1108 continue
    ...
    kxi = kappa*xi; kxe = kappa*xi*eps !! non-dimensional parameters w/o units.
        ...
        case(1) !! Loop
            ...
            allocate(pdfz(ratio_pdf)); allocate(pdft(ratio_pdf,pdft_max)) !! ratio_pdf defines the number of interval. 200 refers to the time section number.
            !allocate(msdx_num(Nmax)); allocate(msdt_num(Nmax)); 
            allocate(sumx(dtmax)); allocate(sumt(dtmax));
            allocate(Dcoefx(Nmax)); allocate(Dcoeft(Nmax))
    

            count_particle = 1 !! Here we start from the 1st particle.
            ...
            !if (MOD(count_particle,int(num_particle/6.0d0)).eq.0) then
            ...
            if (count_sim.le.20) then
                write(382,60) count_particle
                do k=1,ratio_pdf
                    itm_f = (k - 0.5d0)/(ratio_pdf) * rayon * eps !! Just calculate the average height for k-th data. Do not want to define a new varaible.....
                    write(382,782) (k - 0.5d0)/ratio_pdf, 1.0d0*pdfz(k)/Nmax, &
            ...
            do i=1,dtmax/ratio_msd !! Loop for ∆t
                sumx(i) = 0.0d0; sumt(i) = 0.0d0
                do j=1,Nmax - i*ratio_msd !! Loop for t_i
            ...
            do l=1,pdft_max
                ...
                do k=1,ratio_pdf
                    write(383,3832,advance='yes') (k - 0.5d0)/ratio_pdf, 1.0d0*pdft(k,l)!1.0d0*(pdft(k,l)-0.0d0)/num_particle !! Mush add "1.0d0*" else all results = 0.
                end do
            end do
            call cpu_time(time_end)
            ...
            deallocate(velocity); deallocate(position); deallocate(force); deallocate(pdfz); deallocate(pdft)
            !deallocate(msdx_num); deallocate(msdt_num); 
            deallocate(sumx); deallocate(sumt); deallocate(Dcoefx); deallocate(Dcoeft)
