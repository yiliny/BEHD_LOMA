...
Program main
    ...
    real*8 :: itm_q(3,2),itm_v(3,2),itm_f(2),itm_g(6),itm_p(3,2),zsqrt !! Define intermediates to facilitate the codes. 20230911, Position->itm_q & Momenta->itm_p
    ...
    kxi = kappa*xi; kxe = kappa*xi*eps !! non-dimensional parameters w/o units.
        ...
        case(1) !! Loop
            ...
            do i = 1 , ratio_sim * (Nmax - 1)
                ...
                amplitude(1,1)=sqrt((18.0d0*xi)/(beta*zsqrt*itm_q(1,1))*(1.0d0+(63.0d0*kappa*itm_v(1,1))/(32.0d0*itm_q(1,1)**3)))
            ...
            goto 1107
                do l=1,dtmax
                    ...
                    if ((MOD(l,ratio_msd).eq.0).or.(l.le.25)) then !! Only output non-zero results.
                        ...
                        itm_g(1) = gammavalue(1,2,2,ini_height,0.0d0) !! Consider gamma_{0,xx} = gamma_{x0}, function of xi/eps/∆
                        !itm_g(2) = gammavalue(1,3,3,ini_height,0.0d0,0.0d0,0.0d0) !! Consider gamma_{0,ΘΘ} = gamma_{Θ0}, function of xi/eps/∆
                        itm_g(3) = gammavalue(2,2,2,ini_height,0.0d0) !! Consider gamma_{1,xx}, function of kpa/xi/eps/∆
