...
Program main
    ...
    do k=1,node
        ...
        !else
        !    tvl = -GLpoint(node,1,i-node/2); wi = GLpoint(node,2,i-node/2)
        !end if
        ...
        !tvl = tvl * zmax
        !aint = aint + wi * exp(-GBdist(tvl,0) * beta) * reff
    end do
    write(385,*) " ## A = ",aint
    write(*,*) " ## A = ",aint
    ...
    kxi = kappa*xi; kxe = kappa*xi*eps !! non-dimensional parameters w/o units.
        ...
        case(1) !! Loop
            ...
            write(*,961) kappa, ini_height, temp; write(*,962) rayon, rhosty, rhosol; write(*,963) kk, q0, num_particle
